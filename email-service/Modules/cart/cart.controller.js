...
module.exports = addToCart = async (req, res, next) => {
    ...
    cart.subTotal = cart.products.reduce((sum, item) => {
        const prod = item.productId._id.equals(product._id) ? product : item.productId;
...
module.exports = deleteFromTheCart = async (req, res, next) => {
    ...
    if (!productFound) return next(new Error("Product not found in cart", { cause: 404 }));

    // Update cart subtotal
    cart.subTotal = cart.products.reduce((sum, item) => sum + (item.quantity * item.productId.priceAfterDiscount), 0);

    const cartDb = await cart.save();
    if (!cartDb) return next(new Error("Failed to delete from cart", { cause: 400 }));
...
module.exports = showCart = async (req, res, next) => {
    ...
    const cart = await Cart.findOne({ userId }).populate('products.productId', 'name priceAfterDiscount');
    if (!cart) return next(new Error("Cart not found", { cause: 404 }));
...
module.exports = updateCart = async (req, res, next) => {
    ...
    if (!updated) return next(new Error("Product not found in cart", { cause: 404 }));
